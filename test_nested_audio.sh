...
echo ""
echo "Commands during stepping:"
echo "  - Enter: Step to next line"
echo "  - 'v': Read all current variables"
echo "  - 'f': Read function context"
echo "  - 'e': Explore changed variables (then select 0-9, p for next page)"
...
python -m autodebugger run \
    --manual \
    --manual-web \
    --manual-from tests/test_nested_structures.py:10 \
    --manual-audio \
    --manual-rate 210 \
    --stop \
